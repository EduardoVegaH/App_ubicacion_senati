...
class _StudentHomeScreenState extends State<StudentHomeScreen> {
  ...
  final Map<String, DateTime?> _courseFirstEntryTime =
      {}; // Primera vez que ingresó al salón
  final Map<String, bool> _courseMonitoringActive =
      {}; // Si está monitoreando este curso

  //Polígono aproximado del campus SENATI INDEPENDENCIA

  // Polígono de prueba centrado en tu posición actual
  final List<LatLng> campusPolygon = [
    LatLng(-11.997906073637472, -77.0624780466876), // arriba-izquierda
    LatLng(-12.000650779873927, -77.06204828923563), // arriba-derecha
    LatLng(-12.000121147900016, -77.0583019566308), // abajo-derecha
    LatLng(-11.997358627864077, -77.05871520796907), // abajo-izquierda
  ];
  ...
  CourseHistory _generateSampleHistory(
    String courseName,
    String startTime,
    String endTime,
  ) {
    ...
    return CourseHistory(courseName: courseName, records: records);
  }
  ...
  Future<void> _loadStudentData() async {
    ...
    if (data != null) {
      ...
      if (!hasPermissions) {
        ...
        print(
          '💡 El usuario debe habilitar las notificaciones en Configuración > Apps > Senati GPS > Notificaciones',
        );
      }
      ...
      if (student != null && student!.coursesToday.isNotEmpty) {
        print(
          '📚 Programando notificaciones para ${student!.coursesToday.length} cursos',
        );
  ...
  bool pointInsideCampus(double lat, double lon) {
    ...
    for (
      int i = 0, j = campusPolygon.length - 1;
      i < campusPolygon.length;
      j = i++
    ) {
  ...
  Future<void> _updateLocation() async {
    try {
      ...
      final dentro = pointInsideCampus(pos.latitude, pos.longitude);
      print("GPS POS: ${pos.latitude}, ${pos.longitude}");
  ...
  DateTime? _parseTime(String timeStr) {
    try {
      ...
      final result = DateTime(now.year, now.month, now.day, hour, minute);
  ...
  AttendanceStatus _validateAttendanceStatus(
    Course course,
    double? currentLat,
    double? currentLon,
  ) {
    ...
    if (isWithinSchedule ||
        (isAfterEnd && _courseFirstEntryTime[course.name] != null)) {
      ...
      if (isInside) {
        ...
        if (_courseFirstEntryTime[course.name] == null) {
          ...
          return _courseAttendanceStatus[course.name] ??
  ...
  Future<void> _checkCourseAttendance() async {
    ...
    try {
      ...
      for (var course in student!.coursesToday) {
        ...
        final isActive =
            status.status == CourseStatus.inProgress ||
        ...
        if (isActive || status.status == CourseStatus.soon) {
          final attendanceStatus = _validateAttendanceStatus(
            course,
            currentLat,
            currentLon,
          );
                      ...
                      Stack(
                        ...
                        children: [
                          Container(
                            ...
                            child:
                                (student == null || student!.photoUrl.isEmpty)
                                ? Icon(
                                    Icons.person,
                                    size: isLargePhone
                                        ? 42
