...
class PathfindingService {
  ...
  /// 
  /// Retorna una lista de Edges que representan el camino (con sus shapes)
  /// Si no hay camino, retorna una lista vacía
  static List<Edge> findPathAStar({
    ...
    if (startNodeId == endNodeId) {
      // Retornar edge vacío (no hay camino que recorrer)
      return [];
    }
    ...
    while (openSet.isNotEmpty) {
      ...
      if (currentId == endNodeId) {
        ...
        // Convertir lista de IDs a lista de Edges
  ...
  /// Reconstruye la lista de Edges desde la lista de IDs de nodos
  static List<Edge> _reconstructPathEdges(List<String> pathNodeIds, List<Edge> allEdges) {
  ...
  static double _heuristic(MapNode from, MapNode to) {
  ...
  static double calculatePathDistance({
    ...
    for (int i = 0; i < path.length - 1; i++) {
      ...
      if (edge != null) {
        ...
        // Si no hay edge, calcular distancia directa
        final fromNode = nodeMap[fromId];
        final toNode = nodeMap[toId];
        if (fromNode != null && toNode != null) {
          totalDistance += math.sqrt(fromNode.distanceToReal(toNode));
        }
      }
    }
